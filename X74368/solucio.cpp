...
Abin<int> crear_arbre() {
    int x; 
    cin >> x;
    if(x!=-1) {
        Abin<int> ae = crear_arbre();
        Abin<int> ad = crear_arbre();
        Abin<int> arec(ae,x,ad);
        return arec;
    }
    else {
        Abin<int> buit;
...
//print in inorder
void print_ino(Abin<int>::iterador it, Abin<int>::iterador end) {
    //it es arrel, end es final()
    if( it != end ) {
        print_ino(it.fesq(), end);
        cout << " " << *it;
        print_ino(it.fdret(), end);
    }
}

//print in postorder
void print_pos(Abin<int>::iterador it, Abin<int>::iterador end) {
    //it es arrel, end es final()
    if(it != end ) {
        print_pos(it.fesq(),end);
        print_pos(it.fdret(),end);
        cout << " " << *it;
    }
}
...
int main()
{
    Abin<int> arbre;
    arbre = crear_arbre();
    //print_ino(arbre.arrel());
    cout << "pos:";
    print_pos(arbre.arrel(),arbre.final());
    cout << endl;
    cout << "ino:";
    print_ino(arbre.arrel(),arbre.final());
    cout << endl;
    return 0;
}
