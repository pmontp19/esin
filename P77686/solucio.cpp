...
bool es_capicua(int n) {
    llista_itr<int> llista;
    while(n > 0){
        int digit = n%10;
        n /= 10;
        llista.inserir_darrera(digit,llista.primer());
    }
    //llista_itr<int> llista(orig);
    
    llista_itr<int>::iterador inc = llista.primer();
    llista_itr<int>::iterador dec = llista.ultim();
    
    while(inc != llista.indef() and dec != llista.indef()){
        if(*inc != *dec) return false;
        inc++;
        dec--;
    }
    return inc == llista.indef() and dec == llista.indef();
}
